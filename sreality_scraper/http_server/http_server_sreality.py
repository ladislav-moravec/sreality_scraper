...
class SimpleHTTPRequestHandler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        cursor.execute(f"SELECT * FROM sreality LIMIT {rows_per_page} OFFSET {offset}")
        ...
        for row in rows:
            ...
            image_urls_str = row[3]
            image_urls = re.findall(r'\"(.*?)\"', image_urls_str)  # Extract URLs using regex
            first_image_url = image_urls[0].strip() if image_urls else ''  # Get the first URL if exists
